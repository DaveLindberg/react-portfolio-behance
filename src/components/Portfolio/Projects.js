...
class User extends Component {
        ...
        data.projects.forEach(project => {
          ...
          if (!imageBlocked.test(project.covers[202])) {
            let data = {
              name: project.name,
              published_on: () =>
                moment(project.published_on * 1000).calendar(),
  ...
  render() {
    let projects = this.state.projects.map((project, i) => {
      let showFeatured = () =>
        this.props.showFeatured
          ? project.featured() ? 'Card__featured' : ''
          : '';
      ...
      return (
        <article
          className={`Card Grid__cell xs-6 sm-4 md-3 lg-2 ${showFeatured()}`}
          key={i}
        >
          <img className="Card__media" src={project.image} alt="project" />
          <div className="Card__details">
            <div>
              <i className="material-icons md-12">thumb_up</i>{' '}
              {project.stats.appreciations}
            </div>
            <div>
              <i className="material-icons md-12">visibility</i>{' '}
              {project.stats.views}
            </div>
          </div>
    ...
    return <div className="Projects Grid">{projects}</div>;
  }
}
